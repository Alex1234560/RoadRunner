...
@Disabled
@Config
@Autonomous(name = "LEVICODE", group = "Autonomous")
public class LeviCodeRoadrunner extends LinearOpMode {
...
//                    ServoShooter2.setPosition(.32);
    ...
    public void runOpMode() throws InterruptedException {
        ...
        TrajectoryActionBuilder GoBackwardFromIntakeBalls = drive.actionBuilder(new Pose2d(StartingX+49,StartingY+(-18*side) , Math.toRadians(StartingAngle)))
                //grab balls
                //.strafeTo(new Vector2d(StartingX+49, ))
                .lineToY(StartingY+(-14*side))
                ;
        ...
        TrajectoryActionBuilder moveBackFromBase = drive.actionBuilder(initialPose)
                //go to balls
                .strafeTo(new Vector2d(StartingX, StartingY+(-18*side)))
        ...
        for (int i = 0; i<=2; i++) {
            ...
            //Replace this with the distance between the robot and the first line of balls
            double mainDist = 49;
            //Replace this with the distance between ball lines
            double ballDist = 18;

            TrajectoryActionBuilder moveDownField = drive.actionBuilder(initialPose)
                    //go to balls
                    .strafeTo(new Vector2d(StartingX+((-(mainDist+(i*ballDist)))*side), StartingY))

                    ;

            TrajectoryActionBuilder moveUpField = drive.actionBuilder(initialPose)
                    //go to balls
                    .strafeTo(new Vector2d(StartingX+(((mainDist+(i*ballDist)))*side), StartingY))
...
//                        shooter.turnOffShooter()
                            ...
                            intake.intakeOff(),
                            GoBackwardFromIntakeBalls.build(),
